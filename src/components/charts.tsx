...
function pointsFor(values: number[], width: number, height: number, pad = 8): Point[] {
...
export function AreaChart({ values, height = 220, label = '자산 변화 차트' }: AreaChartProps) {
  const width = 760;
  ...
  return (
    <div className="chart-shell" role="img" aria-label={label}>
      <svg viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none">
        ...
        <circle cx={last[0]} cy={last[1]} r="5" className="chart-point" vectorEffect="non-scaling-stroke" />
      </svg>
    </div>
  );
}
...
export function MiniSpark({ values, negative = false }: MiniSparkProps) {
