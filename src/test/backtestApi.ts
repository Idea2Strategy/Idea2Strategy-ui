/*
  ...
  Every payload below was captured from the engine itself: `create_app` was booted
  against B's published `official-backtest-request.valid.json` and the four result
  serialisers (`_run_payload`, `_attempt_payload`, `_performance_payload`,
  `_monthly_payload`, `_manifest_payload`) were rendered over real row dataclasses.
  ...
  fail. The metric values are the ones `test_performance_metrics.py` pins.

  Authentication is modelled the way the server draws the line
  (`backtest_engine/api.py` module docstring): a missing, malformed or unknown
  credential is 401, a valid credential on somebody else's run is 403, and an id that
  belongs to nobody is 404.
*/
...
const DEFAULT_STATE: BacktestApiState = {
  runs: [runPayload()],
  attempts: ATTEMPTS,
...
function principal(request: Request): 'owner' | 'other' | null {
...
export function backtestHandlers(overrides: Partial<BacktestApiState> = {}): RequestHandler[] {
  ...
  return [
    ...
    http.get(path('/:runId/performance'), ({ request, params }) => {
      const runId = String(params.runId);
      const denied = owned(request, runId, state);
    ...
    http.get(path('/:runId/monthly-summaries'), ({ request, params }) => (
      owned(request, String(params.runId), state)
